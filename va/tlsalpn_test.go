...
func TestValidateTLSALPN01BadUTFSrv(t *testing.T) {
	...
	// In go1.16 and greater tlsalpn01Srv is expected to fail because of
	// invalid unicode attempted in the certificate creation. If that
	// error occurs, then the standard library has done it's job and this
	// test is satisfied. If the error is for any other reason, the unit
	// test will fail. In 1.15.x this error is not expected and the other
	// test cases will continue.
	if err != nil {
